---
title: variance
description: "Shows a set of sample values on a number line with the mean \x03bc marked. For each sample it draws the deviation (X-\x03bc) and a corresponding squared-deviation bar, illustrating variance as E[(X-\x03bc)^2]. The right panel animates a linear transform Y=aX+b and compares measured Var(Y) against the rule Var(aX+b)=a^2Var(X), while also showing \x03c3=sqrt(Var)."
date: '2026-07-01'
...
> Source-copy draft imported from [https://templeton.host/visualizations/variance/](https://templeton.host/visualizations/variance/). Names, domain references, and local media paths were adapted for Xavi Ablaza / getablaza.com.
